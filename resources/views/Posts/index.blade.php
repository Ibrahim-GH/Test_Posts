...
@foreach($posts as $post)
            <div class="card" style="width: 18rem;">
                <div class="card-body">
                    ...
                    <img style="width: 90px; height: 90px;" src="{{asset($post->photo)}}" class="card-img-top"  alt="Card image cap">

                </div>
                <a href="{{url('posts/edit', ['post_id' => $post->id])}}" class="btn btn-primary">Edit</a>
                <a href="{{url('posts/delete', ['post_id' => $post->id])}}" class="btn btn-danger">Delete</a>
                <a href="{{url('comments/all', ['post_id' => $post->id])}}" class="btn btn-success">Comments</a>
                <a href="{{url('posts/like', ['post_id' => $post->id])}}" class="btn btn-success">
                    <span class="fa fa-thumbs-up">Like</span></a>
            </div>
